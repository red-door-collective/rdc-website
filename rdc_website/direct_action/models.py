...
events_users = db.Table(
    "events_users",
    db.metadata,
    Column("event_id", db.ForeignKey("events.id"), primary_key=True),
...
phone_bank_tenants = db.Table(
    ...
    Column(
        "phone_bank_event_id",
        db.ForeignKey("phone_bank_events.id", ondelete="CASCADE"),
        primary_key=True,
    ),
    Column(
        "defendant_id",
...
canvass_warrants = db.Table(
    ...
    Column(
        "canvass_event.id",
        db.ForeignKey("canvass_events.id", ondelete="CASCADE"),
        primary_key=True,
    ),
    Column(
