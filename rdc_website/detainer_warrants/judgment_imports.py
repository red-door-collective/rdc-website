...
def _from_workbook(month, court_date, raw_judgment):
    ...
    if judgment[PLAINTIFF]:
        plaintiff, _ = get_or_create(db.session, Plaintiff, name=judgment[PLAINTIFF])
    ...
    judge = None
    if judgment[JUDGE]:
        judge, _ = get_or_create(db.session, Judge, name=judgment[JUDGE])
    ...
    if month < datetime.combine(date(2021, 7, 1), datetime.min.time()):  # July
        ...
        db.session.commit()
        fees_match = FEES_REGEX.search(judgment[NOTES]) if judgment[NOTES] else False
        if fees_match:
            awards_fees = fees_match.group(1)
    # After July, claims were not tracked. only fees awarded.
    else:
        awards_fees = (
            Decimal(str(judgment[AMOUNT]).replace("$", "").replace(",", ""))
            if outcome and "fees" in outcome and judgment[AMOUNT]
            else None
        )

    if outcome:
        in_favor_of = (
            "PLAINTIFF" if "poss" in outcome or "fees" in outcome else "DEFENDANT"
        )
        awards_possession = "poss" in outcome

    basis = judgment[JUDGMENT_BASIS].lower() if judgment[JUDGMENT_BASIS] else None

    mediation_letter = (
        judgment[MEDIATION_LETTER].lower() == "yes"
        if judgment[MEDIATION_LETTER]
        else None
    )
    dismissal_basis = extract_dismissal_basis(outcome, basis)
    notes = judgment[NOTES]

    hearing = Hearing.query.filter_by(
        docket_id=docket_id, _court_date=court_date
    ).first()
    if hearing:
        hearing.update(
            address=address,
    ...
    if hearing.judgment:
        ...
        insert_stmt = insert(Judgment).values(**judgment_values)

        do_update_stmt = insert_stmt.on_conflict_do_update(
            constraint=Judgment.__table__.primary_key, set_=judgment_values
        )

        db.session.execute(do_update_stmt)
        db.session.commit()
        judgment = Judgment.query.filter(
            Judgment._file_date == court_date, Judgment.detainer_warrant_id == docket_id
        ).first()
        hearing.judgment = judgment
        db.session.commit()

    audit_status = (
        "CONFIRMED" if dw.audit_status == "ADDRESS_CONFIRMED" else "JUDGMENT_CONFIRMED"
    )
...
def from_workbook(workbook_name, limit=None, service_account_key=None):
    ...
    worksheets = [
        (dt, wb.worksheet(datetime.strftime(dt, "%B %Y")))
