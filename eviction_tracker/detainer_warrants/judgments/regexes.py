...
DETAINER_WARRANT_SCANNED_PRINTED = re.compile(r"DETAINER\s+SUMMONS")
...
IN_FAVOR_PLAINTIFF = re.compile(r"Order\s*(|)\s*Judgment\s+is\s+granted")
IN_FAVOR_DEFENDANT = re.compile(r"per\s+annum\s*(|)\s*Case\s+is\s+dismissed")
AWARDS = re.compile(
...
INTEREST_FOLLOWS_SITE = re.compile(r"granted as follows:\s*(.+)\s*at the rate posted")
INTEREST_RATE = re.compile(
    r"Courts.\s*(.+)\s*at\s+the\s+rate\s+of\s+%\s*([\d\.]*)\s*per\s+annum"
)

DISMISSAL_FAILURE = re.compile(r"Dismissal is based on:\s*(.+)\s*Failure to prosecute.")
DISMISSAL_FAVOR = re.compile(r"prosecute\.\s*(.+)\s*Finding in favor of Defendant")
