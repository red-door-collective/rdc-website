module Page.Admin.Judges.Edit exposing (Data, Model, Msg, page)
...
getJudge : String -> Int -> Maybe Cred -> Cmd Msg
getJudge domain id maybeCred =
    Rest.get (Endpoint.judge domain id) maybeCred GotJudge (Rest.itemDecoder Judge.decoder)
...
savingError httpError model =
    let
        problems =
            [ ServerError "Error saving judge" ]
    in
    ...
    case msg of
        GotJudge result ->
            case result of
                Ok judgePage ->
                    ( { model
                        | judge = Just judgePage.data
                        , form = Ready (editForm judgePage.data)
                      }
                    , Cmd.none
                    )
        ...
        CreatedJudge (Ok judgeItem) ->
            nextStepSave
                session
                { model
                    | judge = Just judgeItem.data
                }

        CreatedJudge (Err httpError) ->
...
submitForm domain session model =
    ...
    case validate model.form of
        Ok validForm ->
            let
                judge =
                    toJudge model.id validForm
            in
            ( { model
                | newFormOnSuccess = False
                , problems = []
                , saveState = SavingJudge
              }
            , updateJudge domain maybeCred model judge
            )
...
toJudge : Maybe Int -> TrimmedForm -> Judge
toJudge id (Trimmed form) =
            ...
            case model.saveState of
                ...
                Done ->
                    ...
                    , if model.newFormOnSuccess then
                        ...
                        Maybe.withDefault Cmd.none <|
                            Maybe.map (\key -> Nav.replaceUrl key (Url.Builder.relative [ String.fromInt judge.id ] [])) (Session.navKey session)
                    )
...
viewName options form =
    column [ width (fill |> minimum 600), height fill, paddingXY 0 10 ]
        [ viewField
            ...
            , description = [ paragraph [] [ text "This name is how we uniquely identify a Judge." ] ]
...
viewAliases options form =
    column [ width (fill |> minimum 600), height fill, paddingXY 0 10 ]
        [ viewField
            ...
            , description =
                [ paragraph []
                    [ text "These are other names that are used to refer to the same judge." ]
...
viewForm options formStatus =
    case formStatus of
        Initializing id ->
            column [] [ text ("Fetching judge " ++ String.fromInt id) ]

        Ready form ->
            column [ centerX, spacing 30, width (fill |> maximum 1200) ]
                [ tile
                    [ paragraph [ Font.center, centerX ] [ text "Judge" ]
                    , formGroup
                        [ viewName options form
                        ]
                    , formGroup
                ...
                (el
                    ([ Font.size 14
                     ...
                     , Events.onLoseFocus CloseTooltip
                        ...
                        ++ withTooltip JudgeInfo model.tooltip [ paragraph [] [ text "The person sueing a tenant for possession or fees." ] ]
                    )
                    (Button.fromLabel "Help" |> Button.cmd (ChangeTooltip JudgeInfo) Button.primary |> Button.renderElement cfg)
                )
            ]
...
updateJudge : String -> Maybe Cred -> Model -> Judge -> Cmd Msg
updateJudge domain maybeCred model form =
    let
        judge =
            Encode.object
    ...
    case model.id of
        Just id ->
            Rest.itemDecoder Judge.decoder
                |> Rest.patch (Endpoint.judge domain id) maybeCred (toBody judge) CreatedJudge

        Nothing ->
            Rest.post (Endpoint.judges domain []) maybeCred (toBody judge) CreatedJudge (Rest.itemDecoder Judge.decoder)
...
head static =
    Seo.summary
        ...
        , description = "Edit judge details"
        , locale = Just "en-us"
        , title = title
        }
        |> Seo.website
