module Page.Admin.Attorneys.Edit exposing (Data, Model, Msg, page)
...
type alias FormOptions =
    ...
    , originalAttorney : Maybe Attorney
...
editForm : Attorney -> Form
editForm attorney =
    { name = attorney.name
    , displayName = ""
    , aliases = attorney.aliases
...
init pageUrl sharedModel static =
    ...
    , case maybeId of
        Just id ->
            getAttorney domain id maybeCred
...
getAttorney : String -> Int -> Maybe Cred -> Cmd Msg
getAttorney domain id maybeCred =
    Rest.get (Endpoint.attorney domain id) maybeCred GotAttorney (Rest.itemDecoder Attorney.decoder)
...
savingError httpError model =
    let
        problems =
            [ ServerError "Error saving attorney" ]
    in
    ...
    case msg of
        GotAttorney result ->
            case result of
                Ok attorneyPage ->
                    ( { model
                        | attorney = Just attorneyPage.data
                        , form = Ready (editForm attorneyPage.data)
                      }
                    , Cmd.none
                    )
        ...
        CreatedAttorney (Ok attorneyItem) ->
            nextStepSave
                session
                { model
                    | attorney = Just attorneyItem.data
                }

        CreatedAttorney (Err httpError) ->
...
submitForm domain session model =
    ...
    case validate model.form of
        Ok validForm ->
            let
                attorney =
                    toAttorney model.id validForm
            in
            ( { model
                | newFormOnSuccess = False
                , problems = []
                , saveState = SavingAttorney
              }
            , updateAttorney domain maybeCred model attorney
            )
...
toAttorney : Maybe Int -> TrimmedForm -> Attorney
toAttorney id (Trimmed form) =
            ...
            case model.saveState of
                ...
                Done ->
                    ...
                    , if model.newFormOnSuccess then
                        ...
                        Maybe.withDefault Cmd.none <|
                            Maybe.map (\key -> Nav.replaceUrl key (Url.Builder.relative [ String.fromInt attorney.id ] [])) (Session.navKey session)
                    )
...
viewName options form =
    column [ width (fill |> minimum 600), height fill, paddingXY 0 10 ]
        [ viewField
            ...
            , description = [ paragraph [] [ text "This name is how we uniquely identify a Attorney." ] ]
...
viewAliases options form =
    column [ width (fill |> minimum 600), height fill, paddingXY 0 10 ]
        [ viewField
            ...
            , description =
                [ paragraph []
                    [ text "These are other names that are used to refer to the same attorney." ]
...
viewForm options formStatus =
    case formStatus of
        Initializing id ->
            column [] [ text ("Fetching attorney " ++ String.fromInt id) ]

        Ready form ->
            column [ centerX, spacing 30, width (fill |> maximum 1200) ]
                [ tile
                    [ paragraph [ Font.center, centerX ] [ text "Attorney" ]
                    , formGroup
                        [ viewName options form
                        ]
                    , formGroup
...
formOptions cfg model =
    ...
    , originalAttorney = model.attorney
    , renderConfig = cfg
    }
                ...
                (el
                    ([ Font.size 14
                     ...
                     , Events.onLoseFocus CloseTooltip
                        ...
                        ++ withTooltip AttorneyInfo model.tooltip [ paragraph [] [ text "The person sueing a tenant for possession or fees." ] ]
                    )
                    (Button.fromLabel "Help" |> Button.cmd (ChangeTooltip AttorneyInfo) Button.primary |> Button.renderElement cfg)
                )
            ]
...
updateAttorney : String -> Maybe Cred -> Model -> Attorney -> Cmd Msg
updateAttorney domain maybeCred model form =
    let
        attorney =
            Encode.object
    ...
    case model.id of
        Just id ->
            Rest.itemDecoder Attorney.decoder
                |> Rest.patch (Endpoint.attorney domain id) maybeCred (toBody attorney) CreatedAttorney

        Nothing ->
            Rest.post (Endpoint.attorneys domain []) maybeCred (toBody attorney) CreatedAttorney (Rest.itemDecoder Attorney.decoder)
...
head static =
    Seo.summary
        ...
        , description = "Edit attorney details"
        , locale = Just "en-us"
        , title = title
        }
        |> Seo.website
